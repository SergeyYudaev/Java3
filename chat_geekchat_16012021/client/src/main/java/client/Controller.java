...
public class Controller implements Initializable {
                ...
                try {
                    ...
                    while (true) {
                        ...
                        if (str.startsWith("/")) {
                            ...
                            if (str.equals("/никЗбс")) {
                                regController.changeNickOk();
                            }

                            if (str.equals("/никНеЗбс")) {
                                regController.changeNickNo();
                            }
                    ...
                    while (true) {
                        ...
                        if (str.startsWith("/")) {
                            ...
                            Platform.runLater(() -> {
                                if (str.startsWith("/history") && str.split("//")[1].equals(this.nickname)) {
                                    textArea.appendText(str.split("//")[2] + "\n");
                                }
                            });
    ...
    public void clientListClicked(MouseEvent mouseEvent) {
        ...
        textField.setText(String.format("%s %s ", Command.PRVTMSG, reciever));
    }
    ...
    //вот эта штука пытается менять ник
    public void tryToChangeNick(String login, String password, String nickname) {
        if (socket == null || socket.isClosed()) {
            connect();
        }

        String msg = String.format("%s %s %s %s", "/changeNick", login, password, nickname);
        //вот это полетит в ClientHandler
        try {
            out.writeUTF(msg);
        } catch (IOException e) {
            e.printStackTrace();
